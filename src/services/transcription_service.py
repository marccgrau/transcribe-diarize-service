...
def _get_attr_or_key(obj, attr: str, key: str | None = None, default=None):
    """
    Safely fetch a value that might be present as an attribute (obj.attr)
    or as a mapping key (obj[key]). Avoids evaluating a failing fallback.
    """
    if hasattr(obj, attr):
...
def transcribe_audio(
    ...
    # pass 1: phrase-level (no word_timestamps for speed; VAD filter trims silences)
    ...
    # Normalize phrase segments
    segments = []
    full_text = ""
    for s in segments_data:
        start = float(_get_attr_or_key(s, "start"))
        end = float(_get_attr_or_key(s, "end"))
        text = _get_attr_or_key(s, "text") or ""
        text = text.strip()
        segments.append({"start": start, "end": end, "text": text})
        if text:
            full_text += text + " "

    # pass 2: word timestamps (prefer WhisperX alignment when lang supported)
    word_segments = []
    ...
    if use_align and segments:
        try:
            ...
            logger.warning(
                "Alignment failed for %s: %s. Falling back to whisper word timestamps.",
                detected,
                e,
            )
            # fall back to direct word timestamps
                ...
                if words:
                    for w in words:
                        word_segments.append(
                            {
                                "word": _get_attr_or_key(w, "word"),
                                "start": float(_get_attr_or_key(w, "start")),
                                "end": float(_get_attr_or_key(w, "end")),
                            }
                        )
        finally:
            try:
                del align_model  # noqa: F821
            except Exception:
                pass
